...
## Oneof
如果一个消息包含许多可选字段，并且最多同时设置一个字段，那么可以强制执行此行为并使用oneof功能节省内存。`Oneof`字段类似于`optional`字段，所有的字段共享一块内存，并且最多同时可以设置一个字段。设置任何一个字段都会覆盖其它字段值。根据所选编程语言，使用`case()`或者`WitchOneof()`方法赋值。

### 使用Oneof
要在`.proto`中定义oneof，使用`oneof`关键字：
...
```
然后，将此字段添加到一个消息中。可以添加任何类型的字段，但不能使用`required`，`optional`或者是`repeated`关键字。
在生成的代码中，其中一个字段具有与常规`optional`方法相同的getters和setters。在所选编程语言的API中，可以通过一个特殊的方法来检查该方法被赋了哪个值。

### Oneof特点
- 对字段赋值将自动清除oneof的其它成员。所以如果多次赋值，只有最后一次赋值生效：
...
- 如果解析器遇到同一个oneof的多个成员，那么只有最后一个成员的值将被解析。
- Extensions不支持oneof。
- oneof不能是`repteated`。
- 反射API适用于其中一个字段。（翻译问题）
- 如果使用C++语言，一定要确保代码不会导致内存崩溃。以下示例代码将导致泄漏。原因是已经通过set_name  ()方法删除了sub_message。
